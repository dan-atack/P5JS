// Display and size settings

const WORLD_WIDTH = 64 // World width in blocks
const WORLD_HEIGHT = 32 // World height in blocks
const SIDEBAR_WIDTH = 256 // Sidebar width in pixels
const BLOCK_WIDTH = 16 // Block width in pixels
const PLAYER_WIDTH = 32 // Player wheel / chassis dimensions
const ECONOMY_TICK_INTERVAL = 120   // Standardize the amount of game ticks between economic updates (game is ~ 60 ticks per second)
const RESCUE_COUNTDOWN_IN_TICKS = 380 // Amount of tick intervals until the game ends in victory for the player

// Colours, in ROYGBIV order (and then grays from lighter to darker)

const RED_ROCK = '#B13A1A';
const OXYGEN_RED = '#800000';
const NEGATIVE_RED = '#FF0000';
const TOGGLED_RED = '#583C36';
const POWER_YELLOW = '#FFDE89';
const YELLOW_SAND = '#FFA500';
const CO2_COLLECTOR_YELLOW = '#B28000';
const BRIGHT_GREEN = '#66FF00';
const FOOD_GREEN = '#7CA059';
const GREENHOUSE_GREEN = '#475B33';
const BLUEGREEN = '#0CBAA6';
const BLUE_ICE = '#AED8E5';
const SOLAR_PANEL_BLUE = '#012846';
const GLASS_FACTORY_BLUE = '#0478D2';

const AIRY_GRAY = '#FAFAFA';
const SIDEBAR_GRAY = '#AAAEBA';
const MINERAL_GRAY = '#82777C';
const SMELTER_GRAY = '#0F0F0F';
const DARK_GRAY = '#3A3B3C';




